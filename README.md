...
Six of the seven suites are written and one is not, and `tests/README.md` says which and why. There were five when this directory was scoped; the two that were added came from reading the clause list inward rather than outward from the probes, which is the direction that finds what nothing checks. What they check is a specific subset of the constitution: **capabilities that must be absent, and behaviours that must be refused.** Most of the constitution is not mechanically checkable, and the tests do not pretend otherwise.
