...
Thirteen suites, all written, and `tests/README.md` says what each covers. They run against any implementation over HTTP and import nothing from one; the reference is `atarasy/valence`. Re-measured in full on 2026-09-11, after the two conformance roles, the store and the passkey's assertion: **184 mutations, 202 declarations, 216 probes at runtime (one skipped), 206 shown to fail, 10 not shown to fail, 0 survivors, 0 inert, and 3 that abort before a probe runs.** Count the numerator and the denominator over the same population: `coverage.sh` unions failure lines from the conformance logs and from the engine's own unit tests, and that union was **236 lines, of which 30 are the engine's own tests** out of 62; the 206 is conformance probes alone. `tests/MUTATIONS.md` names the ten that have never been shown to fail, one by one. **Five of the ten are new and the cause is the harness**: seeding the role-split pair runs under `set -e`, so a mutation that breaks the seed kills the run before any probe starts, and `roles_are_swapped`, `registry_needs_both_roles` and `require_registered_merchant` now abort. Four `roles/` probes and one `registry/` probe are proven by nothing as a result. `scripts/fragility.py` reports how much each proof rests on: of 209 probes with a catch, **66 are caught by exactly one mutation** and 8 rest only on a mutation that spans nine or more suites.
