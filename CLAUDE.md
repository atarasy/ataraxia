...
Eleven suites, all written, and `tests/README.md` says what each covers. They run against any implementation over HTTP and import nothing from one; the reference is `atarasy/valence`. Re-measured in full on 2026-09-10, after the delivery wall and the merchant-export probes went in: **173 mutations, 188 declarations, 202 probes at runtime, 197 shown to fail, 5 not shown to fail, 0 survivors, and 1 inert that was found and fixed.** The inert one was this session's own doing: a new field in the candidate view sat inside another mutation's anchor. Count the numerator and the denominator over the same population: `coverage.sh` unions failure lines from the conformance logs and from the engine's own unit tests, and **21 of the 218 lines in that union are the latter**, which leaves 197 conformance probes. `tests/MUTATIONS.md` names the five that have never been shown to fail, one by one: two are proven only by the two mutations excluded for breaking the shared fixture, two are properties of this deployment, and one is a boundary reached by another mutation first. `scripts/fragility.py` reports how much each proof rests on: of 194 probes with a catch, **64 are caught by exactly one mutation** and 4 have no catch but a mutation that spans nine or more suites.
