...
describe("exit: the move (clause 52)", () => {
  ...
  test("a set that arrives with no confirmation cannot be taken back (§16.5, §14.2)", async () => {
    ...
    //
    // §16.5 tells a set nobody signed by the absence of a recorded
    // confirmation (question 43), and a move is a claim by the sending host
    // rather than a proof. So a decided offer that arrives without its
    // register is, to the receiving host, a set nobody signed, and the route
    // that needs no signature must not return it to `presented`.
    const house = encodeURIComponent(household());
    const offer = await createConformingOffer({ household: household() });
    await call("POST", `/offers/${offer.id}/present`, {});
    const decisions = offer.candidates.map((c) => ({
