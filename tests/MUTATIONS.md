...
| `carried_marker_not_migrated` | A store written before the carried-settlement marker left unmarked on first open, so a payment a recipient's import planted comes back into its giver's record on upgrade (§14.2, question 66) | Isolated run 2026-09-19: caught by 1 engine unit test, which opens an SQLite file written without the marker. No conformance probe can: it is a property of a store across an upgrade |
| `migration_trusts_an_empty_ledger` | The marker pass run wherever the ledger says it keeps its rows, even when it holds none for this store's settlements, which marks every settlement a host made before the reference ledger was persisted (§14.2, question 66) | Isolated run 2026-09-19: caught by 1 engine unit test, which drops the `reservations` table a store from before 2026-09-19 never had |
