...
| `withdraw_a_set_nobody_signed` | A set with no confirmation recorded can be taken back (§16.5, questions 43 and 50). **What it still reaches is a set that arrived by a move without its register**, in the digital binding or on a physical box with no recorded collection before its expiry: the set returns to `presented` and can be decided again with the confirmation it arrived without (§10.5). The collected box question 43 was written for is refused first by the question 46 guard. **It survived the sweep of 2026-09-15** for that reason, and was read the same day as unreachable before the move was found to reach it. Question 50 named the register in §14 and moved the export to `valence-node/6`; the probe in `exit` imports a `/5` node without the register and fails alone under this mutation, with 422 where 409 was expected | 1 (`exit`) |
...
| `import_mandate_is_live` | A mandate that arrives by a move written as a mandate rather than as a claim (§14.2, question 56, decided 2026-09-18), so a row nobody signed carries that household's protections and an offer may name it. The import route authenticates nobody: measured the day it was decided, a stranger planted a second mandate under a household's own identifier with a ceiling of 9,999,999 beside its real one at 0 | Isolated run 2026-09-18: caught by the engine's `names` unit test for a claim an offer cannot name, and by seven others |
| `record_ignores_the_claims_version` | The version a claim arrived at ignored when the household signs it (§14.2, §16.1, question 56), so a mandate amended where it came from can never become one here: its holder is asked for a version 1 they never agreed to | Isolated run 2026-09-18: caught by the engine's `names` unit test for signing a claim at the version it carries |
| `claim_is_not_held_for_the_household` | A claim left out of what a household holds (§16.2, §14.2, question 56), so a move drops every protection instead of failing closed: the receiving host reads the household as one that has set nothing and leaves every offer alone | Isolated run 2026-09-18: caught by the engine's `names` unit test and by three others |
