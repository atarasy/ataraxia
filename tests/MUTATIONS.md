...
| `remote_mandates_fail_open` | A hub's answer about a household's mandates read as "there are none" whenever it is not a clean `has: true` (§16.2), so a transport failure, a 404 from an older hub or a 200 of the wrong shape all drop the protection. **Found by the third refutation pass, which measured `200 {}` reading as none** | Isolated run pending; caught by the engine's `mandate-source` unit test, which is where it lives: no suite can make a hub answer badly |
| `remote_mandate_any_body` | Whatever a hub answers read as a mandate (§13.2). A body of `{}` has no lapse and no ceiling, so every comparison against it is false and the protections apply to nothing | Isolated run pending; caught by the engine's `mandate-source` unit test |
