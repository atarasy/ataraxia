...
| ~~`withdraw_a_set_nobody_signed`~~ | **Retired 2026-09-15.** It removed the guard refusing to withdraw a set the household never signed (§16.5, question 43). **It survived the sweep of that day, and no probe was missing.** A box decided with no confirmation was decided either by a collection, which the question 46 guard above it refuses first, or by the deadline, which runs only past expiry, where the question 47 guard refuses. No input reaches the guard, so removing it changes nothing. The guard stays in the engine so that loosening either of the other two does not silently reopen question 43, and the probe in `binding` still asserts the refusal whichever guard gives it | retired, the guard is unreachable |
