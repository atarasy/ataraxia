...
| `settle_cooling_reads_the_named_mandate` | The cooling window at settlement read from the mandate the offer names rather than the longest the household holds (§16.5, question 68) | Isolated run 2026-09-19: caught by 1 conformance probe, `permissions: ... > a second mandate does not shorten the cooling window either (§16.5)` |
| `withdraw_cooling_reads_the_named_mandate` | The same at the other end of the window, so a set under a second label cannot be taken back inside the window the household set on its first (§16.5, question 68) | Isolated run 2026-09-19: caught by 2 conformance probes, the one above and `permissions: ... > a signed set on a box past its expiry cannot be taken back (§16.5)` |
