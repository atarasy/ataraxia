...
| `import_mandate_identical_refused` | A held mandate refused even when the import carries the same mandate (§14.2, question 52), so a node whose mandate a hub already recorded cannot move | Isolated run 2026-09-15: caught by the engine's `import-scope` unit test; no probe yet |
...
| `import_mandate_looser_taken` | A later mandate version taken whatever it says (§16.1, question 52), so a move loosens a mandate the host holds without the co-signers a change needs | Isolated run 2026-09-15: caught by the engine's `import-scope` unit test; no probe yet |
...
| `import_mandate_older_version_taken` | An older mandate version taken over a newer one (§16.1, question 52), which puts a captured version back in play at the version after it. **Found by the fourth refutation pass** | Isolated run 2026-09-15: caught by the engine's `import-scope` unit test; no probe yet |
| `import_mandate_fields_unchecked` | A mandate's ceilings, window, lapse and version left unchecked on import (§16.1, question 52), so `lapses_at: "never"` imports and no read finds the mandate lapsed. **Found by the fourth refutation pass** | Isolated run 2026-09-15: caught by the engine's `import-scope` unit test; no probe yet |
