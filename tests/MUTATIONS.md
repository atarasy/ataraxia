...
| `decision_recheck_dropped` | The check after `decide`'s wait on the mandate source dropped, so two decisions of one set in flight are both applied and two confirmations recorded (§10.5). The wait came with the lapse rules on 2026-09-19. **No conformance probe reaches it**: the reference's in-memory source answers at once, so two decisions over HTTP never overlap | Isolated run 2026-09-19: caught by 1 engine unit test, `lapse > two decisions in flight over one set: one is applied and the other refused` |
