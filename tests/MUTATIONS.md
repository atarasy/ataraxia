...
| `ceiling_not_enforced` | An offer presented over the ceiling the person signed. Failed no probe on its first run: nothing placed one | 1. Re-anchored 2026-09-19 when questions 65 and 66 rewrote its line, which a first refutation pass found had left it unable to apply, and re-run in isolation: caught by 3 conformance probes and 2 engine unit tests |
| `lapsed_mandate_still_works` | An offer presented on a mandate that had lapsed. Failed no probe on its first run: nothing used one | 1. Re-anchored 2026-09-19 when questions 65 and 66 rewrote its line, which a first refutation pass found had left it unable to apply, and re-run in isolation: caught by 1 conformance probe and 1 engine unit test |
...
| `settlement_is_not_reported` | Settle without telling the person's own copy. The daily ceiling then measures nothing on a split deployment and the household's export loses the settlement. **Everything still answers**, which is why it needs a probe that reads one party after writing to the other | 1. Re-anchored 2026-09-19 when questions 65 and 66 rewrote its line, which a first refutation pass found had left it unable to apply, and re-run in isolation: caught by 1 conformance probe and 5 engine unit tests |
...
| `import_mandate_replaces_held` | A mandate the host already holds replaced by an import from its own household (§14.2, question 52), which drops co-signers that loosening needs (§16.1) | Isolated run 2026-09-15: caught by the engine's `import-scope` unit test; no probe yet Re-measured 2026-09-20: the route-only mutation survived because importMandate independently rejects a held identifier. Retargeting both guards fails the existing same-household replacement assertion (283 unit pass, one fail). No conformance catch is claimed. |
...
| `import_keeps_a_lapsed_claim` | A claim kept although it has already lapsed (§14.2, question 56), which `record` refuses as lapsed for ever, so the identifier holds a row nobody can act on. The import route's own shape check admits `lapses_at: 0` | Isolated run 2026-09-18: caught by the engine's `names` unit test Re-measured 2026-09-20: the admission-only mutation survived because claimFor independently hides expired claims. Retargeting admission and that lookup fails two names unit assertions (already-lapsed claim and later lapse); 282 unit pass, two fail, 329 conformance pass and one expected skip. No conformance catch is claimed. |
...
| `gift_ceiling_is_the_recipients` | A gift's daily ceiling read from the recipient's mandate while the giver pays (§12, §16.3, question 60) | Isolated run 2026-09-19: caught by 2 engine unit tests, and after a second refutation pass noted that nothing else caught it, by `silence/silence.test.ts` > a gift is held to the giver's daily ceiling, and the recipient's never refuses it. Re-anchored 2026-09-19 when the lapse rules fixed a decided set's window and ceiling at its decision, so that it breaks the read at the decision as well as the one at settlement (a break of one alone leaves the other enforcing the rule), and re-run in isolation: caught by 2 engine unit tests and 1 conformance probe, `silence: the ceremonial default > a gift is held to the giver's daily ceiling, and the recipient's never refuses it (§12, §16.3)` |
| `gift_day_counted_to_recipient` | A gift's charge counted to the recipient's day while the giver pays (§12, §16.3, question 60) | Isolated run 2026-09-19: caught by 1 conformance probe, first `binding: lost is not billed to the household (§3.2) > a lost candidate is reported and is not charged` and 1 engine unit test. Re-anchored 2026-09-19 when questions 65 and 66 rewrote its line, which a first refutation pass found had left it unable to apply, and re-run in isolation: caught by 1 engine unit test |
| `remote_ceiling_silence_is_none` | A hub that answers without `ceiling_daily` read as one saying the giver set no ceiling (§16.3, question 60) | Isolated run 2026-09-19: caught by 1 engine unit test. Re-anchored 2026-09-19 when questions 65 and 66 rewrote its line, which a first refutation pass found had left it unable to apply, and re-run in isolation: caught by 1 engine unit test |
...
| `zero_settle_meets_the_ceiling` | A settlement of nothing refused on a day already past the ceiling, trapping a set that owes nothing behind its reserve (§16.3, question 62) | Isolated run 2026-09-19: caught by 1 engine unit test. Rewritten the same day: once the ceiling stopped being read for a zero charge, restoring the comparison alone changed nothing and the mutation SURVIVED; it now restores both halves and is caught again. Re-anchored 2026-09-19 when the lapse rules fixed a decided set's window and ceiling at its decision, so that it breaks the read at the decision as well as the one at settlement (a break of one alone leaves the other enforcing the rule), and re-run in isolation: caught by 1 engine unit test, `a declined gift settles at nothing without asking for the giver's ceiling`. The test written for it, `a box collected with nothing used settles at nothing at the export, on a day already past the ceiling`, does not catch it, because its stub answers no daily ceiling; that was so before the lapse rules too |
...
| `report_before_settled` | The day told before the offer is marked settled, and a settlement whose offer was never marked left as it is: a failed report leaves an offer nothing can withdraw, settle or move (§6.4, §13.2, question 62) | Isolated run 2026-09-19: caught by 1 engine unit test. Re-anchored 2026-09-19 when questions 65 and 66 rewrote its line, which a first refutation pass found had left it unable to apply, and re-run in isolation: caught by 1 engine unit test |
...
| `zero_settle_asks_the_giver` | The giver's ceiling asked for even when nothing is charged, so a hub that predates question 60 refuses a declined gift (§16.3, question 60) | Isolated run 2026-09-19: caught by 1 engine unit test. Re-anchored 2026-09-19 when the lapse rules fixed a decided set's window and ceiling at its decision, so that it breaks the read at the decision as well as the one at settlement (a break of one alone leaves the other enforcing the rule), and re-run in isolation: caught by 1 engine unit test |
| `lapsed_mandate_governs_the_giver` | A lapsed mandate's ceiling governing a giver (§16.3, question 60) | Isolated run 2026-09-19: caught by 1 engine unit test. Re-anchored 2026-09-19 when questions 65 and 66 rewrote its line, which a first refutation pass found had left it unable to apply, and re-run in isolation: caught by 1 engine unit test |
...
| `own_offer_reads_its_named_ceiling` | The out-of-network ceiling of a household's own offer read from the mandate the offer names, as before question 68 (clause 47, §16.2). This is the escape itself: a household that had tightened one label presents under a second it recorded alone | Isolated run 2026-09-19: caught by 1 conformance probe, `permissions: mandates: the thresholds a person sets are enforced > a second mandate does not raise the ceiling the household set on another (clause 47, §16.2)` |
| `own_settle_reads_its_named_daily_ceiling` | The daily ceiling read for a gift's giver alone, as question 60 left it, so a household's own settlement reads the mandate the offer names and a second label removes the ceiling (§16.3, question 68) | Isolated run 2026-09-19: caught by 1 conformance probe, `permissions: ... > a second mandate does not raise the daily ceiling either (§16.3)`. Re-anchored 2026-09-19 when the lapse rules fixed a decided set's window and ceiling at its decision, so that it breaks the read at the decision as well as the one at settlement (a break of one alone leaves the other enforcing the rule), and re-run in isolation: caught by 1 conformance probe, `permissions: ... > a second mandate does not raise the daily ceiling either (§16.3)` |
| `settle_cooling_reads_the_named_mandate` | The cooling window at settlement read from the mandate the offer names rather than the longest the household holds (§16.5, question 68) | Isolated run 2026-09-19: caught by 1 conformance probe, `permissions: ... > a second mandate does not shorten the cooling window either (§16.5)`. Re-anchored 2026-09-19 when the lapse rules fixed a decided set's window and ceiling at its decision, so that it breaks the read at the decision as well as the one at settlement (a break of one alone leaves the other enforcing the rule), and re-run in isolation: caught by 1 conformance probe, `permissions: ... > a second mandate does not shorten the cooling window either (§16.5)` |
| `withdraw_cooling_reads_the_named_mandate` | The same at the other end of the window, so a set under a second label cannot be taken back inside the window the household set on its first (§16.5, question 68) | Isolated run 2026-09-19: caught by 2 conformance probes, the one above and `permissions: ... > a signed set on a box past its expiry cannot be taken back (§16.5)`. Re-anchored 2026-09-19 when the lapse rules fixed a decided set's window and ceiling at its decision, so that it breaks the read at the decision as well as the one at settlement (a break of one alone leaves the other enforcing the rule), and re-run in isolation: caught by 1 conformance probe, the cooling one above |
| `cooling_takes_the_shortest` | The shortest window among a household's mandates taken instead of the longest (§16.5, question 68). The tightest window is the longest one, and this is the one place in §16 where the tighter value is the larger | Isolated run 2026-09-19: caught by 2 conformance probes, the cooling one above and the hub's household route, and by 1 engine unit test |
| `lapsed_mandate_governs_own_offer` | A lapsed mandate's out-of-network ceiling governing a household's own offers (clause 58, §16.2, question 68) | Isolated run 2026-09-19: caught by 1 engine unit test, `mandate-source > the local source reads the tightest out-of-network ceiling and the longest window`. **No conformance probe reaches it**: a probe would have to wait out a lapse on one mandate while another stays live |
| `lapsed_cooling_governs_own_offer` | A lapsed mandate's cooling window governing, so a set waits behind a window the household stopped renewing (clause 58, §16.5, question 68) | Isolated run 2026-09-19: caught by the same engine unit test, and by no conformance probe, for the same reason |
| `hub_household_drops_out_of_network` | The hub's household route answered without the tightest out-of-network ceiling, which an engine holding no register reads there (§13.1, §16.2, question 68) | Isolated run 2026-09-19: caught by 1 conformance probe, `permissions: ... > the hub answers a household's protections, not its rows (§13.1, §16.2, §16.3, §16.5)` |
| `hub_household_drops_cooling` | The same route answered without the longest cooling window (§13.1, §16.5, question 68) | Isolated run 2026-09-19: caught by 2 conformance probes, the one above and `roles: the two parties are one implementation (§13.2) > a settlement on the engine reaches the person's own copy on the hub`, which is the split deployment reading it over §13.1 |
| `remote_out_of_network_silence_is_none` | A hub that answers without `ceiling_out_of_network` read as one saying the household set none, which presents an offer past the ceiling it set on another of its labels (§16.2, question 68). **No conformance probe can reach it**: no route produces a hub that answers without a field, so the rule is proven in `engine/test/` or not at all | Isolated run 2026-09-19: caught by 1 engine unit test, `mandate-source > the remote source reads both, and silence is not none either` |
| `remote_cooling_silence_is_none` | The same for `cooling_seconds`, which settles a set the household could still take back (§16.5, question 68) | Isolated run 2026-09-19: caught by the same engine unit test |
| `lapse_forward_is_the_households_alone` | Bringing a co-signed mandate's lapse forward counted as a tightening again, so the household alone lets its tight mandate lapse and a loose label it recorded alone governs every offer (§16.1, decided 2026-09-19 after the first refutation pass over question 68) | Isolated run 2026-09-19: caught by 1 conformance probe, `permissions: ... > bringing a co-signed mandate's lapse forward needs the co-signer (§16.1)`, and 1 engine unit test, `lapse > the household alone cannot bring it forward, and so cannot let a loose label govern` |
| `lapse_unbounded` | A version's lapse taken any distance out, so a mandate naming a co-signer nobody holds binds every label of its household until the year 9999 (§16.1, clause 58, decided 2026-09-19 after the first refutation pass over question 68) | Isolated run 2026-09-20 at the widened bound: caught by 1 conformance probe, `permissions: ... > a mandate lapses at most 400 days after it is recorded (§16.1, clause 58)`, and 1 engine unit test, `lapse > a household that has lost its co-signer is held for at most 400 days` |
| `lapse_bound_is_a_year` | The bound back at 366 days, a year plus the minutes between two clocks, so a member whose device is two days fast is refused every change to their own protections, tightenings included (§16.1, clause 58, widened to 400 on 2026-09-20 after the second refutation pass over question 68). The reference hub computes the lapse on the device, and every button on that screen writes a whole version | Isolated run 2026-09-20: caught by 1 conformance probe, `permissions: ... > a mandate lapses at most 400 days after it is recorded (§16.1, clause 58)`, and 1 engine unit test, `lapse > a device a week fast still records every kind of change (§16.1)` |
| `cooling_unbounded` | A cooling window recorded at any length (§16.5, decided 2026-09-20 after the second refutation pass over question 68). A decided set keeps the window it was decided under, so thirty years recorded, a set decided under it and the window dropped a second later leaves that set unable to settle until 2056, under a mandate showing no window at all | Isolated run 2026-09-20: caught by 1 conformance probe, `permissions: ... > a cooling window is at most 30 days (§16.5)`, and 1 engine unit test, `lapse > the thirty-year trap is refused at the record` |
| `protection_has_no_floor` | A ceiling or a cooling window recorded below zero, or as a fraction, at every caller that is not the HTTP route (§16.1, decided 2026-09-20 after the third refutation pass over question 68). The floor was `requireInteger(..., 0)` at the route, and the register is what the deployment acceptance flow and every test call; a negative window is not an absent one, so §16.5's longest selects it wherever it is a household's only one and the screen shows a window no engine can honour. **No conformance probe reaches it**: the route refuses the same values `400 malformed` before the register sees them, so `not_a_protection` is a register rule proven in `engine/test/` | Isolated run 2026-09-20: caught by 1 engine unit test, `lapse > the register refuses what only the HTTP route refused` |
| `engine_reads_any_window` | The engine reading whatever a source answers for a cooling window (§16.1, §16.5, decided 2026-09-20 after the third refutation pass over question 68). `MAX_COOLING_SECONDS` was enforced at `MandateRegister.record` alone, so a hub that is not this register, and a row written before the bound existed, still fixed a decided set for thirty years; the reader reads an out-of-range value as the nearest one in range rather than refusing, because refusing turns an old row into a settlement that can never be made and blocks that presenter's next box. **No conformance probe reaches it**: the suites' hub is this register, which now refuses the value at the writing end | Isolated run 2026-09-20: caught by 1 engine unit test, `lapse > a source answering outside the bounds is read at the nearest value inside them` |
| `decide_refuses_a_silent_hub` | A decision refused again because the mandate source could not answer (§16.3, §16.5, decided 2026-09-20 after the second refutation pass over question 68). Against a hub built one day earlier, which answers the household route without `cooling_seconds`, a household's refusal of a gift is refused `hub_refused`, the offer reaches its expiry with every line still `offered`, §12 defaults them, and the giver is charged 700 for goods the recipient declined. **No conformance probe reaches it**: the reference's local source answers every read, and the suites' engine-only deployment is pointed at a hub that is up | Isolated run 2026-09-20: caught by 1 engine unit test, `old-hub > a household's refusal of a gift is not turned into a purchase` |
| `one_silence_drops_the_other` | One catch over both reads at a decision instead of one each, so a hub that answers `ceiling_daily` and not `cooling_seconds`, which is every hub predating question 68, records neither and a set decided under a daily ceiling of 500 is charged past it once that mandate lapses (§16.3, decided 2026-09-20) | Isolated run 2026-09-20: caught by 1 engine unit test, `old-hub > one silence does not drop the other reading` |
| `unreachable_hub_is_no_ceiling` | A hub that cannot be reached read as one saying the household has set no protection, so an offer entirely outside the network presents past the ceiling it set (§13.1, §16.2). `hub_unreachable` is named in the specification and was asserted nowhere, in the suites or in `engine/test/`, until 2026-09-20. **No conformance probe reaches it**: producing an engine whose hub is down needs a deployment the conformance contract does not ask an implementation for | Isolated run 2026-09-20: caught by 1 engine unit test, `old-hub > a hub that is not there refuses a presentation and a settlement, and not a decision` |
| `refused_hub_is_no_ceiling` | The other half of the same sentence: a hub that answers and refuses read as one saying the household has set none (§13.1, §16.2). `hub_refused` was asserted nowhere until 2026-09-20, for the same reason | Isolated run 2026-09-20: caught by the same engine unit test |
| `decide_falls_back_to_nothing` | A read that fails at a decision applying no protection at all, instead of the last reading this host had from the source for that household (§16.3, §16.5, decided 2026-09-20 after the third refutation pass over question 68). The pass measured one silent read at one moment taking away a day-long cooling window and a ceiling of 500 and settling 900: on a split deployment the engine is the presenter's side and the decision arrives through it, so **the read that has to fail belongs to the party that profits from it failing**. **No conformance probe reaches it**: the suites' engine-only deployment is pointed at a hub that is up, and a hub that fails on cue is outside what a probe has | Isolated unit run 2026-09-20 at valence `42af979`: caught by 2 engine unit tests, `§16.3, §16.5: a hub that cannot answer does not refuse a decision > an outage at the decision leaves the last reading standing`; `§16.3, §16.5: a hub that cannot answer does not refuse a decision > a household this host has never read cannot decide, and that is what the rule costs`. Conformance was not run on this isolated copy. |
| `decide_proceeds_with_nothing_read` | A decision made although this host has never read anything from the source for that household, which is a set decided under no protection at all and the one case the fallback cannot cover (§16.3, decided 2026-09-20). **No conformance probe reaches it**, for the reason above | Isolated unit run 2026-09-20 at valence `42af979`: caught by 1 engine unit test, `§16.3, §16.5: a hub that cannot answer does not refuse a decision > a household this host has never read cannot decide, and that is what the rule costs`. Conformance was not run on this isolated copy. |
| `decision_hides_a_failed_read` | The record not saying which of its values came from an earlier reading (§16.5, decided 2026-09-20). The pass named the silence as unreadable anywhere: a household saw `no_cooling` and could not tell it from a mandate that set no window. **No conformance probe reaches it** | Isolated unit run 2026-09-20 at valence `42af979`: caught by 1 engine unit test, `§16.3, §16.5: a hub that cannot answer does not refuse a decision > an outage at the decision leaves the last reading standing`. Conformance was not run on this isolated copy. |
| `withdraw_takes_any_caller` | `DELETE /offers/{id}/decisions` taking any caller again, so whoever holds the offer id voids a decision the household signed (§16.5, decided 2026-09-20 after the third refutation pass over question 68). Each half was known and correct: §16.1 recorded in 2026-09-11 that the route asked for no signature, and question 68 made the window the longest across every mandate a household holds. **The window is the interval in which the route is open**, so the change lengthened it, and the pass measured a recipient's written refusal of a gift turned into a 1,200 charge to its giver | Isolated unit run 2026-09-20 at valence `42af979`: caught by 2 engine unit tests, `§16.5: a signed set is taken back by a signature over taking it back > a caller holding the offer id cannot turn a written refusal into a purchase`; `§16.5: a signed set is taken back by a signature over taking it back > the household's own take-back still works, and a re-decision needs its own`. Conformance was not run on this isolated copy. |
| `statement_asks_for_a_window` | The cooling window read before the settlement asks whether it can bar anything, so a hub predating question 68 refuses a statement settlement `hub_refused`, the statement stays unsigned and §6.5 blocks that presenter's next box (§16.5, decided 2026-09-20 after the third refutation pass). Measured by the pass on a household holding no mandate at all. **No conformance probe reaches it**: it needs a hub that answers without a field | Isolated unit run 2026-09-20 at valence `42af979`: caught by 1 engine unit test, `§16.3, §16.5: a hub that cannot answer does not refuse a decision > an old hub does not stop a box the household has signed for`. Conformance was not run on this isolated copy. |
| `export_drops_decided_protections` | Drops the recorded protections from a node export, so a move loses the record (§14, §16.5). | Isolated unit run 2026-09-20 at valence `42af979`: caught by 1 engine unit test, `§14.2, §16.5: what a decided set was decided under moves with it > the record is exported with the offer and arrives at the next host`. Conformance was not run on this isolated copy. |
| `import_protections_unscoped` | Accepts a protection record for an offer the import does not carry (§14.2). | Isolated unit run 2026-09-20 at valence `42af979`: caught by 1 engine unit test, `§14.2, §16.5: what a decided set was decided under moves with it > a record naming an offer the import does not carry is refused`. Conformance was not run on this isolated copy. |
| `settle_window_not_fixed` | A set settled against the live window alone, so a lapse since the decision ends a window already running (§16.5, decided 2026-09-19 after the first refutation pass over question 68) | Isolated run 2026-09-19: caught by 1 conformance probe, `permissions: ... > a lapse does not end the window of a set already decided (§16.5)`, and 2 engine unit tests in `lapse` |
| `withdraw_window_not_fixed` | A set taken back against the live window alone, so a lapse since the decision refuses the take-back `no_cooling` (§16.5, decided 2026-09-19) | Isolated run 2026-09-19: caught by the same conformance probe and the same 2 engine unit tests |
| `settle_daily_not_fixed` | A set settled against the live daily ceiling alone, so a lapse since the decision removes the ceiling it was decided under (§16.3, decided 2026-09-19) | Isolated run 2026-09-19: caught by 2 conformance probes, `permissions: ... > a lapse does not remove the daily ceiling of a set already decided (§16.3)` and `... > a box its collection decided keeps the daily ceiling live at the collection (§16.3, §11.2)`, and 2 engine unit tests |
| `decision_fixes_nothing` | A household's decision records nothing, so a set it signed is held to the live mandates alone (§16.3, §16.5, decided 2026-09-19) | Isolated run 2026-09-19: caught by 2 conformance probes, the window and the daily ones above, and 3 engine unit tests |
| `collection_fixes_nothing` | A collection that decides a box records nothing, so the box settles days later against the live ceiling alone (§16.3, §11.2, decided 2026-09-19). Since 2026-09-20 the rule is `collect`'s and not the route's, so an in-process caller meets it: the engine unit test below calls `engine.collect` directly, which is the path the service behind api-dev.vox.delivery takes | Isolated run 2026-09-20: caught by 1 conformance probe, `permissions: ... > a box its collection decided keeps the daily ceiling live at the collection (§16.3, §11.2)`, and 1 engine unit test, `lapse > a collection that decides a box fixes what is live at its moment, in process` |
| ~~`collection_route_fixes_nothing`~~ | **Retired 2026-09-20.** It made the route call a synchronous `collect` that fixed nothing, which was a real break only while the fixing lived on a `collectDeciding` beside it. The second refutation pass over question 68 named that split as the defect: `collect` decided a box and recorded nothing, and the service behind api-dev.vox.delivery calls `collect`. The fixing moved into `collect`, so there is no second path left to take, and `collection_fixes_nothing` is now the whole rule | retired, no behaviour left to break |
| `decision_recheck_dropped` | The check after `decide`'s wait on the mandate source dropped, so two decisions of one set in flight are both applied and two confirmations recorded (§10.5). The wait came with the lapse rules on 2026-09-19. **No conformance probe reaches it**: the reference's in-memory source answers at once, so two decisions over HTTP never overlap. The probe added on 2026-09-20 asserts the refusal's name, which the other check also produces, and so does not depend on reaching the race | Isolated run 2026-09-19: caught by 1 engine unit test, `lapse > two decisions in flight over one set: one is applied and the other refused` |
| `decide_writes_before_the_day` | The record of a decided set's protections written before the offer is reported to the household's day, which is the last thing in `decide` that can fail, so a day source that throws leaves `decided_protections` on the disk of a set the store still holds as `presented` (§10.5, measured by the second refutation pass over question 68). **No conformance probe reaches it**: a hub that fails on cue and a look at the durable store are both outside what a probe has | Isolated run 2026-09-20: caught by 1 engine unit test, `lapse > a day source that throws leaves no record behind` |
...
| `gift_names_no_host` | A gift signed over bytes that name no host (§12, question 58) | Isolated run 2026-09-19: caught by 7 conformance probes and 2 engine unit tests; the re-run after the probe fix below measured 7 conformance probes and 1 engine unit test. The first row credited `silence: ... a gift's signature names the host, and its giver is a key`, and a first refutation pass measured that probe passing under this mutation: it asked only for a refusal, which a form without a host also gives. It now presents the same terms signed for this host, and re-run in isolation the same day it fails under the mutation |
...
| `gift_out_of_network_is_the_recipients` | The recipient's out-of-network ceiling read for a gift the giver pays (clause 46, §16.2, question 65) | Isolated run 2026-09-19: caught by 1 conformance probe, `mandates: a loosening needs its co-signers (clauses 46, 47, §16) > a gift is held to its giver's out-of-network ceiling and not its recipient's (clause 46, §16.2)`, and 3 engine unit tests |
| `given_line_counts_out_of_network` | A line a maker gave counted towards the out-of-network ceiling, though nobody pays for it (clauses 10 and 46, question 65) | Isolated run 2026-09-19: caught by 1 engine unit test |
| `settle_not_serialised` | Two settles of one payer run at once, so both read the day before either is counted (§16.3, question 66) | Isolated run 2026-09-19: caught by 1 engine unit test. No conformance probe can: the reference's in-memory ledger resolves every await at once, so two settles over HTTP are serialised by scheduling |
| `retry_does_not_report` | A settle retried after a failed report answers with the settlement and tells the day nothing (§16.3, question 66) | Isolated run 2026-09-19: caught by 1 engine unit test |
| `made_here_read_from_the_ledger` | A reserve on the ledger taken as proof a settlement was made here, which a ledger with memory-only holds forgets on restart (§12, §14, question 66) | Isolated run 2026-09-19: caught by 1 engine unit test |
| `carried_settlement_is_a_payment` | Every settlement of a gift listed as its giver's payment, including one a recipient's import carried (§12, §14, question 61) | Isolated run 2026-09-19: caught by 1 engine unit test. Re-anchored the same day when the marker replaced the ledger's reserve, and re-run: caught by the same test |
| `store_predating_the_marker_is_silent` | A store opened without writing what it can say about itself, so a store that predates the record of where a settlement was made is never labelled, and one opened empty is never labelled as having the record from the start (§14.2, question 66) | Isolated run 2026-09-19: caught by 1 engine unit test. Rewritten the same day when the carried marker became the positive record `settled_here` and the label moved to first open, and re-run: CAUGHT. The two marker-migration mutations that stood here were retired with the pass they broke: two refutation passes showed a reservation's absence is not evidence that a move carried a settlement |
| `settled_mark_written_after` | The settlement written before the mark that says this host made it, so a failure between the two leaves one this host made unmarked and its day never told (§14.2, question 66) | Isolated run 2026-09-19: caught by 1 engine unit test, which fails the second write with an SQLite trigger and restarts before the retry |
| `predating_store_says_nothing` | A store that predates the record opened without telling its operator (§14.2, question 66) | Isolated run 2026-09-19: caught by 1 engine unit test. The sixth refutation pass found the warning removable with the suite green |
| `settled_report_unauthenticated` | The engine's credential check on `POST /households/{id}/settled`, so a stranger can fill or zero a household's day (§13.2, §16.3, question 67) | Isolated run 2026-09-22: caught by 3 engine unit tests and the `roles/` probe that a report without the engine's credential is refused and counts for nothing |
| `offer_report_unauthenticated` | The same check on `POST /households/{id}/offers`, so a stranger can write rows into the person's copy (§13.2, question 67) | Isolated run 2026-09-22: caught by 1 engine unit test and the same `roles/` probe |
| `report_credential_any_bearer` | The comparison with the engine's credential, so any `Bearer` value is taken (question 67) | Isolated run 2026-09-22: caught by 1 engine unit test and the same `roles/` probe |
| `report_credential_unset_accepts` | The refusal on a hub given no credential, so an unset credential means anyone, which is the shape the route had before (question 67) | Isolated run 2026-09-22: caught by 1 engine unit test only. **No probe reaches it**: the harness gives both role servers a credential and the combined server never exposes the route to an engine report |
| `remote_day_omits_credential` | The credential on the engine's two reports, so every settlement it makes is refused by a hub in another process (question 67) | Isolated run 2026-09-22: caught by 1 engine unit test and 2 `roles/` probes, the end-to-end settlement and the hub's export |
| `contact_not_in_canonical_form` | The merchant's contact stops reaching the bytes a merchant signs (§10a, question 72), so a signature no longer covers which contact is shown beside a block and a tampered contact still verifies | Isolated run 2026-09-22: caught by 1 engine unit test, which signs a contact and then reads a tampered one as still verified |
| `approval_drops_contact` | The approval, the screen a person signs from, drops the merchant's contact off its disclosure blocks while the offer still carries it (§10a.4, question 72) | Isolated run 2026-09-22: caught by 1 conformance probe, `disclosure: a merchant's contact (§10a, question 72) > a signed contact comes back on the approval exactly as signed`, and 1 engine unit test |
| `contact_kind_unchecked` | `kind` stops being checked against the three allowed values, so an unrecognised kind falls to the `url` branch and a caller naming one beside a well-formed https value is registered rather than refused (§10a, question 72) | Isolated run 2026-09-22: caught by 1 conformance probe, `disclosure: a merchant's contact (§10a, question 72) > a contact of an unknown kind is refused, even where the value alone would pass as a url`, and 1 engine unit test |
| `contact_rendered_as_null` | The absent contact on the approval screen, rendered as `contact: null`, which a client from before question 72 reads as malformed because it checks a block's keys exactly (§10a requirement 7) | Isolated run 2026-09-22: caught by 1 engine unit test |
| `offer_view_contact_as_null` | The same on `GET /offers/{id}` (§10a requirement 7) | Isolated run 2026-09-22: caught by 1 engine unit test |
