...
| `record_takes_the_claims_version_alone` | A claim's version taken for the claim (§14.2, §16.1, clause 47, question 56), so a household records **its own** terms at that version and drops the co-signers it named where the mandate came from: clause 47 escaped by relocation | Isolated run 2026-09-18: caught by the engine's `names` unit test for a claim being an offer to sign and never a constraint |
| `import_takes_an_offer_in_progress` | An offer taken although it arrives still in progress (§14.2, §6.4, question 57, decided 2026-09-18), so an offer that can be decided here holds no reservation on this ledger and §6.4's upper bound is one this ledger has never seen. Measured: the import took one at `presented` with a `201` and no reserve | Isolated run 2026-09-18: caught by the engine's `names` unit test for a move carrying what happened, and by one other |
