...
| `withdraw_forgets_missing` | §16.5, question 46. A cooling window resets a line the collection recorded missing, so the household can sign `kept` over goods reported gone | Isolated run 2026-09-14: caught by the engine's tests |
...
| `withdraw_resets_the_collection` | Withdrawal resets collection-owned valences as well as household-signed decisions. Re-measured on 2026-09-13: it survived the existing 117 unit and 279 conformance tests. A new mixed-box unit test keeps consumed and returned lines unchanged while resetting the signed kept line; removing the guard fails that assertion directly | Isolated corrected coverage: 1 unit failure out of 118 unit tests; 279 conformance passes and 1 skip. Not a full-sweep count |
...
| `block_counts_unsettleable` | The unsigned-statement block counts a box nobody can settle: one back in `presented` after its collection, where `settle` is a `409`, and one a presenter withdrew, which can never settle at all. Either makes a household unofferable by every presenter on that engine with no act available to it that would lift the block. **Its probe was rewritten 2026-09-14**: question 46 refuses the partial collection it used, which had made it vacuous, so it now reopens the box through the household's cooling window and then withdraws it | Isolated rerun 2026-09-14: caught, and 2 unit tests |
