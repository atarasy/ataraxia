...
| `import_register_after_edges` | The register taken in after the edges rather than with each offer (§14.2, question 50), so an import refused at an edge leaves its offers written with no register and a retry cannot add it. **Found by the third refutation pass over question 50**; the reference import is not atomic, which is the defect underneath | Isolated run 2026-09-15: caught by the `exit` probe for an offer written before a later row is refused |
