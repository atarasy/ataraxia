...
| `an_action_never_matches_its_household` | The live action is found and then judged to belong to another household, so every grant is refused by the check meant to catch a borrowed one. Second break beside `refuse_every_grant`, which loses the action rather than reading it wrongly | 6 |
...
| `refusal_names_the_offer` | The refusal names the waiting box, so one presenter learns another's offer id and `GET /offers/{id}` serves it: products, prices, merchant, and which lines that household used (clause 8). §16.3 already sets the shape, which is that a refusal says only that it refused | 1, and 1 unit test |
...
**All fourteen were run the same night, one at a time, and all fourteen were caught; none survived, none was inert, none aborted.** Clean on the branch afterwards: **257 tests across 14 files, 256 passing and one skipped**, and `anchors.py` reading **277 scripts with 0 drifted**. Two of the fourteen are caught by the engine's own tests and by no probe, and both say why in their row.
