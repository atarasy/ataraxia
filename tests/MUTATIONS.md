...
| `collection_fixes_nothing` | A collection that decides a box records nothing, so the box settles days later against the live ceiling alone (§16.3, §11.2, decided 2026-09-19) | Isolated run 2026-09-19: caught by 1 conformance probe, `permissions: ... > a box its collection decided keeps the daily ceiling live at the collection (§16.3, §11.2)`, and 1 engine unit test |
| `collection_route_fixes_nothing` | The collection route goes through the synchronous `collect`, which fixes nothing (§16.3, §11.2, decided 2026-09-19). The engine's unit tests call the engine directly, so only a probe reaches the route | Isolated run 2026-09-19: caught by 1 conformance probe, the collection one above |
