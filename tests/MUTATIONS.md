...
| `import_edge_replaces_held` | An imported edge allowed to replace one the host holds (§14.2, question 52). The signed bytes carry no id, so an import under one household signed an edge of its own under another household's edge's id and erased it. **Found by the second refutation pass over question 52** | Isolated run 2026-09-15: caught by the engine's `import-scope` unit test; no probe yet |
