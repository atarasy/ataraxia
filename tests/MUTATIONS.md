...
| `store_predating_the_marker_is_silent` | A store that predates the carried-settlement marker upgraded in silence, so nobody is told it holds settlements it cannot tell apart (§14.2, question 66) | Isolated run 2026-09-19: caught by 1 engine unit test. The two marker-migration mutations that stood here were retired the same day with the pass they broke: two refutation passes showed a reservation's absence is not evidence that a move carried a settlement |
