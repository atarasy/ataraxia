...
**All fifteen were run the same night, one at a time, and all fifteen were caught; none survived, none was inert, none aborted.** Clean on the branch afterwards: **257 tests across 14 files, 256 passing and one skipped**, and `anchors.py` reading **278 scripts with 0 drifted**. Three of the fifteen are caught by the engine's own tests and by no probe, and each says why in its row.
