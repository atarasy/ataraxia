...
Clause 42 makes the person's own agent the default recipient of their data,
...
of a permission. Clause 41 removes it from the settings screen, and a ledger
...
field means unlimited, then unlimited is expressible, and clause 41's time
limit is a convention rather than a structure.

**A revocation that removes the row**: clause 44 says the list is always
...
**A price on a permission**: clause 43 says no capability to sell data is
built. One column is the capability.

**A model named on a row**: clause 45 says changing the model behind an agent
